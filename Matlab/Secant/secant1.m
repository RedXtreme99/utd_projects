% A solution for electronic problem 3.3.6.a.
...
% Function for f(x) = x^3 - 3x + 1
function f = func(x)
    f = x.^3 - 3*x + 1;
end

% Function for f'(x) = 3x^2 - 3
...
function rootn = newtons(x, nmax, eps, sigma)
    ...
    for n=1:nmax
        ...
        if abs(d) < eps
            Newtons_Output_a = array2table(output, 'VariableNames', {'Iteration', 'x', 'f(x)'})
            rootn = x
            return
        end
    end
